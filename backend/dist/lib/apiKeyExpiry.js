...
export function readReminderSettings(env = process.env) {
...
export function msUntilNextRun(now, hourIst) {
...
export function reminderCutoff(now, days) {
    return new Date(now.getTime() + days * DAY_MS);
}
export function isDueForReminder(key, now, days) {
    if (key.status !== 'active')
        return false;
    if (!key.expires_at)
        return false;
    if (key.expiry_reminder_sent_at)
        return false;
