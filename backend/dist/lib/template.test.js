...
const send = (html, data = {}) => applyDeferredStyles(renderTemplate(htmlToPlainText(html, { styleWithUnicode: true }), data));
...
test('renderTemplate escapes substituted values for email', () => {
    assert.equal(renderTemplate('<p>{{name}}</p>', { name: '<script>' }, { escapeHtml: true }), '<p>&lt;script&gt;</p>');
});
test('renderTemplate leaves a variable inside an attribute usable', () => {
    assert.equal(renderTemplate('<a href="{{url}}">x</a>', { url: '/i/9' }, { escapeHtml: true }), '<a href="/i/9">x</a>');
});
test('validateVariables names every missing key', () => {
    assert.throws(() => validateVariables(['a', 'b'], { a: 1 }), (err) => err instanceof MissingVariablesError && err.missing.join() === 'b');
