...
export async function createMessageLog(input) {
    ...
    const result = await db.collection('message_logs').insertOne(log);
...
export async function markFailedAndScheduleRetry(logId, attemptsSoFar) {
    ...
    await db.collection('message_logs').updateOne({ _id: logId }, { $set: { status: 'pending', attempts, next_retry_at: nextRetryAt, updated_at: new Date() } });
}
