...
function styleRun(text, active) {
    let out = text;
    const bold = active.has('bold');
    const italic = active.has('italic');
    if (bold && italic)
        out = toUnicodeStyle(out, 'boldItalic');
    else if (bold)
        out = toUnicodeStyle(out, 'bold');
    else if (italic)
        out = toUnicodeStyle(out, 'italic');
    if (active.has('underline'))
        out = addCombining(out, UNDERLINE);
    if (active.has('strike'))
        out = addCombining(out, STRIKE);
    return out;
}
...
const FLAGS = { bold: 'b', italic: 'i', underline: 'u', strike: 's' };
const STYLES = { b: 'bold', i: 'italic', u: 'underline', s: 'strike' };
/**
 ...
 */
export function applyStyles(text, active) {
    if (!text || active.size === 0)
        return text;
 ...
 */
export function applyDeferredStyles(text) {
    if (!text.includes(OPEN))
        return text;
    return text
        .replace(DEFERRED, (_full, flags, value) => {
