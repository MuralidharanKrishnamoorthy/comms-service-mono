...
categoriesRoute.get('/', async (c) => {
    ...
    return c.json(categories.map((cat) => ({ ...cat, template_count: countByCategory.get(cat._id.toString()) ?? 0 })));
});
...
categoriesRoute.get('/:categoryId/projects/:projectId/templates', async (c) => {
    ...
    const [templates, links] = await Promise.all([
        db.collection('templates').find({ project_id: new ObjectId(projectId) }).toArray(),
        db
            .collection('template_categories')
...
categoriesRoute.post('/:categoryId/projects/:projectId/templates/:templateKey', async (c) => {
    ...
    const [category, template] = await Promise.all([
        db.collection('categories').findOne({ _id: new ObjectId(categoryId) }),
        db.collection('templates').findOne({ project_id: new ObjectId(projectId), template_key: templateKey }),
    ]);
    if (!category)
        return c.json({ error: 'Category not found' }, 404);
    if (!template)
    ...
    try {
        await db.collection('template_categories').insertOne(link);
    }
    catch (err) {
...
categoriesRoute.delete('/:categoryId/projects/:projectId/templates/:templateKey', async (c) => {
    ...
    if (!(await hasProjectAccess(c.get('user'), projectId)))
        return c.json({ error: 'You do not have access to this project' }, 403);
    const db = getDb();
    await db.collection('template_categories').deleteOne({
        category_id: new ObjectId(categoryId),
        project_id: new ObjectId(projectId),
        template_key: templateKey,
    });
