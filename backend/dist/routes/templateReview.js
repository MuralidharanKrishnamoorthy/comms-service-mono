...
import { rejectTemplateSchema, TEMPLATE_STATUS_FILTERS, } from '../models/template.js';
...
export const templateReviewRoute = new Hono();
// GET /templates
...
templateReviewRoute.get('/', async (c) => {
    ...
    const creatorIds = [...new Set(templates.map((t) => t.created_by?.toString()).filter(Boolean))].map((id) => new ObjectId(id));
...
templateReviewRoute.patch('/:id/approve', requireAdmin, async (c) => {
    ...
    const result = await db
        .collection('templates')
        .findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { ...fields, updated_at: new Date() } }, { returnDocument: 'after' });
    if (!result)
