...
import { rejectTemplateSchema, returnTemplateSchema, TEMPLATE_STATUS_FILTERS, } from '../models/template.js';
...
templateReviewRoute.get('/', async (c) => {
    ...
    if (projectParam) {
        if (!ObjectId.isValid(projectParam))
    ...
    if (statusRaw && statusRaw !== 'all') {
        if (!TEMPLATE_STATUS_FILTERS.includes(statusRaw)) {
...
templateReviewRoute.patch('/:id/approve', requireAdmin, async (c) => {
    ...
    const result = await getDb()
        ...
        .findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { ...fields, updated_at: now } }, { returnDocument: 'after' });
    if (!result)
