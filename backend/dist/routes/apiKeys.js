...
import { createApiKeySchema, DEFAULT_EXPIRY_DAYS } from '../models/apiKey.js';
...
apiKeysRoute.post('/', async (c) => {
    ...
    const result = await getDb().collection('api_keys').insertOne(key);
    return c.json({
...
apiKeysRoute.get('/:keyId/reveal', async (c) => {
    ...
    console.info(`[api-keys] reveal: user=${user._id.toString()} key=${key._id.toString()} project=${projectId} at=${new Date().toISOString()}`);
