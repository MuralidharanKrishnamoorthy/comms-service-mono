...
import {
  ...
  reminderHtml,
  reminderSubject,
} from '../lib/apiKeyExpiry.js'
import { sendViaProvider } from '../providers/httpProvider.js'
import type { ApiKey } from '../models/apiKey.js'
import type { Project } from '../models/project.js'
import type { User } from '../models/user.js'

const WITHOUT_SECRETS = { value_encrypted: 0, key_hash: 0 } as const

interface Directory {
  projectName: (id: ObjectId) => string
  creatorEmail: (id: ObjectId | null | undefined) => string | undefined
...
async function runSweep(days: number): Promise<void> {
  const now = new Date()
...
async function loadDirectory(due: ApiKey[]): Promise<Directory> {
  const db = getDb()

  const unique = (ids: (ObjectId | null | undefined)[]) => {
    const seen = new Map<string, ObjectId>()
    for (const id of ids) if (id) seen.set(id.toString(), id)
    return [...seen.values()]
  }
...
async function remindOne(key: ApiKey, days: number, directory: Directory): Promise<void> {
  const keys = getDb().collection<ApiKey>('api_keys')

  const claimed = await keys.findOneAndUpdate(
    { _id: key._id, status: 'active', expiry_reminder_sent_at: null },
    { $set: { expiry_reminder_sent_at: new Date(), updated_at: new Date() } },
    { returnDocument: 'after', projection: WITHOUT_SECRETS }
  )
  if (!claimed?.expires_at) return
  ...
  if (recipients.length === 0) {
    console.error(
      `No one to warn about API key ${claimed._id!.toString()} — its creator is inactive and no active admin exists`
    )
    return
  }
  ...
  console.log(
    `Warned ${recipients.length} recipient(s) that API key ${claimed.key_prefix}… expires ${claimed.expires_at.toISOString()}`
  )
}
