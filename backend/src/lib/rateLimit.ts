...
export interface RateLimiter {
  reserve(quotas: RateLimitQuota[]): (() => void) | null
...
export function createRateLimiter(windowMs: number): RateLimiter {
  ...
  const within = (key: string, now: number): number[] =>
    (hits.get(key) ?? []).filter((at) => now - at < windowMs)

  const prune = (now: number): void => {
    for (const [key, recorded] of hits) {
      if (recorded.every((at) => now - at >= windowMs)) hits.delete(key)
    }
  }
  ...
  return {
    reserve(quotas) {
      ...
      if (current.some(({ recorded, max }) => recorded.length >= max)) {
        for (const { key, recorded } of current) hits.set(key, recorded)
        return null
      }
      ...
      return () => {
        for (const { key } of current) {
          ...
          if (recorded.length === 0) hits.delete(key)
        }
      }
    },
